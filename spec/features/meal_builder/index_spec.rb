...
RSpec.describe 'visiting the meal builder' do
  describe 'meal building with foods' do
    before(:each) do
  ...
  it 'has a user error message and returns to meal builder if no food selected' do
    allow_any_instance_of(ApplicationController).to receive(:current_user_id).and_return(1)
    visit meal_builder_path
    ...
    expect(page).to have_content('Unable to create meal. Please add a food!')
    expect(current_path).to eq(meal_builder_path)
  end
end
